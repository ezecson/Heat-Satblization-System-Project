...
return ADCW;
 ...
 char keypad();
 int calculterror(int r,int x);

  int calculttemp(int x);
...
TCCR0=(1<<WGM00) | (1<<COM01) | (1<<COM00) | (1<<WGM01) | (0<<CS02) | (0<<CS01) | (0<<CS00);
TCNT0=0x00;
OCR0=0x00;
...
while (1)
      ...
      // Place your code here     
                  ch=read_adc(0x00); 
                  temperature=calculttemp(ch);
                  itoa(temperature,temper); 
        if (same != ch ) 
        {
        same=ch; 
        lcd_clear();
